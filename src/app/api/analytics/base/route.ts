...
export async function GET() {
  ...
  const [chainsRaw, historyRaw, stableRaw, feesRaw, dexOverview, ...protocolResults] =
    await Promise.all([
      ...
      fetchBaseDexOverview(),
      ...BASE_ANALYTICS_PROTOCOLS.map(async (meta) => {
        const [tvlUsd, dexBase] = await Promise.all([
          fetchProtocolBaseTvl(meta.slug),
          meta.category === "DEX" ? fetchDexBaseVolume(meta.slug) : Promise.resolve(null),
        ]);
        return { meta, tvlUsd, dexBase };
      }),
    ]);
  ...
  const protocols: ProtocolTvlRow[] = protocolResults
    .map(({ meta, tvlUsd }) => ({
      ...
      tvlUsd,
      defillamaUrl: `https://defillama.com/protocol/${meta.slug}`,
    }))
    .filter((row) => row.tvlUsd !== null && row.tvlUsd > 0)
    .sort((a, b) => (b.tvlUsd ?? 0) - (a.tvlUsd ?? 0));

  const dexRows: DexVolumeRow[] = protocolResults
    .filter(({ meta, dexBase }) => meta.category === "DEX" && dexBase?.total24h)
    .map(({ meta, dexBase }) => ({
      slug: meta.slug,
      name: meta.name,
      ...
      defillamaUrl: `https://defillama.com/summary/dexs/${meta.slug}`,
    }))
    .sort((a, b) => b.volume24h - a.volume24h);

  if (dexRows.length === 0 && dexMetas.length > 0) errors.push("dex_protocols");

  const payload: BaseAnalyticsPayload = {
    updatedAt: new Date().toISOString(),
    source: "defillama",
    chain: {
      name: "Base",
      chainId: baseChain?.chainId ?? 8453,
      tvlUsd: baseChain?.tvl ?? 0,
      rank: baseRank,
