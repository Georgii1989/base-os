...
function Sparkline({ values, positive }: { values: number[]; positive: boolean }) {
  ...
  return (
    <svg viewBox="0 0 100 88" className="h-24 w-full rounded-2xl bg-white/5">
      <defs>
        <linearGradient id="baseTvlFill" x1="0" y1="0" x2="0" y2="1">
      ...
      <path d={`${path} L 100 88 L 0 88 Z`} fill="url(#baseTvlFill)" />
      <path
        d={path}
        fill="none"
...
export function BaseAnalyticsPanel() {
  const { data, isLoading, isError, dataUpdatedAt, refetch, isFetching } = useQuery({
    ...
    queryFn: async (): Promise<BaseAnalyticsPayload> => {
      const res = await fetch("/api/analytics/base");
  ...
  if (isError || !data) {
    return (
      <div className="rounded-3xl border border-rose-300/30 bg-rose-500/10 p-6 text-rose-100">
        <p className="font-bold">Could not load Base analytics.</p>
        <p className="mt-2 text-sm text-rose-200/90">DeFi Llama may be slow — try again in a moment.</p>
        <button
          type="button"
          onClick={() => refetch()}
  ...
  const tvlValues = data.tvlHistory.map((p) => p.tvl);
  const tvlPositive = (data.tvlChange30dPct ?? 0) >= 0;
  ...
  const dexPositive = (data.dexVolume?.change1dPct ?? 0) >= 0;
    ...
    <div className="grid gap-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-300/90">
            TVL, DEX volume, stablecoins, fees, and protocols — live from{" "}
            <a
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <MetricCard
          ...
          accent={dexPositive ? "fuchsia" : "amber"}
        />
        <MetricCard
          label="TVL · 30d"
          value={formatPct(data.tvlChange30dPct)}
          hint="From DeFi Llama history"
          accent={tvlPositive ? "emerald" : "fuchsia"}
        />
        <MetricCard
          label="Stablecoins"
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <section className="rounded-3xl border border-cyan-300/20 bg-slate-950/50 p-5">
          <div className="flex items-baseline justify-between gap-2">
            <h3 className="text-lg font-black text-cyan-100">TVL history</h3>
            <span className="text-xs text-slate-500">~90d sampled</span>
          </div>
          <div className="mt-4">
            <Sparkline values={tvlValues} positive={tvlPositive} />
          </div>
          <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-400">
            <span>
              Start <span className="font-bold text-slate-200">{formatUsd(tvlValues[0])}</span>
            </span>
            <span>
              Now <span className="font-bold text-slate-200">{formatUsd(tvlValues.at(-1))}</span>
            </span>
          </div>
        </section>
            ...
            {data.dexVolume.byProtocol.map((row) => (
              <li key={row.slug}>
                <div className="mb-1 flex justify-between gap-2 text-xs">
                  ...
                  <span
                    className={
                      (row.change1dPct ?? 0) >= 0 ? "text-emerald-300" : "text-rose-300"
                    }
                  >
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="rounded-3xl border border-white/10 bg-slate-950/50 p-5">
          ...
          {data.protocols.length === 0 ? (
            <p className="mt-4 text-sm text-slate-500">Protocol breakdown loading slowly — refresh later.</p>
          ) : (
            <ul className="mt-4 space-y-2">
