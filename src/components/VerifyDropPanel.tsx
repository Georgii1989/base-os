    ...
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      <div className="os-panel p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="os-eyebrow text-violet-200/90">Verify Drop</p>
        ...
        <p className="mt-3 max-w-2xl text-sm text-slate-200/85">
          A working replica of the{" "}
          <a
            href="https://github.com/base/base-verify-demo"
            target="_blank"
            rel="noreferrer"
          ...
          flow: prove ownership of a social account, get a{" "}
          <span className="text-amber-200">deterministic token</span>, claim {config?.amountLabel ?? "the drop"}{" "}
          once — no matter how many wallets you connect.
        </p>
        {isSandbox ? (
          <p className="mt-2 max-w-2xl text-xs text-slate-400">
            Real Base Verify API access is gated by an interest form, so this demo ships a sandbox
            verifier: account stats and tokens are derived deterministically from the handle. Set{" "}
        ...
        {!isConnected ? (
          ...
          <div className="mt-6 grid gap-5">
            ...
            {isSandbox ? (
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  In production this step redirects to{" "}
                  <span className="font-mono text-slate-400">verify.base.dev</span> — here you type a
                  handle and the sandbox derives its account stats deterministically.
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <input
                    value={handleInput}
                ...
                {linkedProfile && linkedEvaluation ? (
                  ...
                  >
                    ...
                    >
                      {linkedEvaluation.satisfied
                        ? "Meets the drop requirements — ready to claim."
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-1 text-sm text-slate-300">
                  Live mode: if this wallet has no verification yet, the claim returns 404 and you
                  should complete OAuth at{" "}
                  <span className="font-mono text-slate-200">verify.base.dev</span>, then claim again.
                </p>
              </div>
            )}
            ...
            <div>
              ...
              >
                {isClaiming ? "Claiming…" : `Claim ${config?.amountLabel ?? "drop"}`}
              </button>
